  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-foreground">Shoot Schedule</h3>
          <p className="text-sm text-muted-foreground">Plan your shooting days</p>
        </div>
        <Button onClick={addShootDay} className="bg-primary hover:bg-primary/90 text-primary-foreground shadow-glow-sm hover-glow">
      ...
      {shootDays.length === 0 ? (
        <Card variant="glass" className="glow-primary">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground mb-4">No shoot days scheduled</p>
            <Button onClick={addShootDay} variant="outline" className="border-border text-foreground hover:bg-secondary/50">
              <Plus className="w-4 h-4 mr-2" />
              Add First Shoot Day
            </Button>
          </CardContent>
        ...
        <div className="space-y-4">
          {shootDays.map((day) => (
            <Card key={day.id} variant="glass" className="hover-glow transition-all duration-300">
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl bg-primary/20 text-primary flex items-center justify-center font-bold text-lg shadow-glow-sm">
                      {day.day_number}
                    </div>
                    <div>
                      <Input
                        ...
                        className="bg-secondary/50 border-border/50 text-foreground h-8"
                      />
                    </div>
                  </div>
                  <div className="flex gap-1">
                    <Button size="sm" variant="ghost" onClick={() => saveShootDay(day)} className="text-muted-foreground hover:text-green-400 hover-glow">
                      <Save className="w-4 h-4" />
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => deleteShootDay(day.id)} className="text-muted-foreground hover:text-destructive">
                ...
                <div className="mt-4">
                  ...
                  <Textarea
                    ...
                    className="bg-secondary/50 border-border/50 text-foreground text-sm resize-none mt-1"
                    rows={2}
